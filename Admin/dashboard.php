    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row d-flex justify-content-center">
          <?php 
            // I ADDED GROUP BY THIS DATE: NOVEMBER 28, 2022
            $academic = mysqli_query($conn, "SELECT sort_Id FROM track_sort WHERE user_Id != '' AND track_answer='ACADEMIC' GROUP BY user_Id");
            $row_academic2 = mysqli_num_rows($academic);

            $tvl = mysqli_query($conn, "SELECT sort_Id FROM track_sort WHERE user_Id != '' AND track_answer='TVL' GROUP BY user_Id");
            $row_tvl = mysqli_num_rows($tvl);

            $sports = mysqli_query($conn, "SELECT sort_Id FROM track_sort WHERE user_Id != '' AND track_answer='SPORTS' GROUP BY user_Id");
            $row_sports = mysqli_num_rows($sports);

            $arts_design = mysqli_query($conn, "SELECT sort_Id FROM track_sort WHERE user_Id != '' AND track_answer='ARTS AND DESIGN' GROUP BY user_Id");
